...
def gather_performance_custom(model_path, env_name, transform_func=None,
    ...
    for depth in tqdm(depths, desc="Training Trees"):
        ...
        for s in tqdm(seeds, desc=f"Evaluating Depth {depth}", leave=False):
            ...
            mr, std = evaluate_tree(eval_env, clf, transform_func=transform_func,
...
def main():
    ...
    OUTPUT_DIR_CUSTOM = "decision_tree_models_gpt"
    ...
    performance_filename_custom = os.path.join(OUTPUT_DIR_CUSTOM, run_folder, "performance_gpt_features.joblib")
    ...
    csv_filename = os.path.join(OUTPUT_DIR_CUSTOM, run_folder, "performance_gpt_features.csv")
    ...
    plt.grid(True)
    plt.legend()
    plot_filename = os.path.join(OUTPUT_DIR_CUSTOM, run_folder, "compare_mean_reward_original_vs_gpt.png")
