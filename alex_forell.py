...
def gather_performance(model_path, env_name, scaler, pca, selected_features,
                       num_samples=10000, n_episodes=10, seeds=list(range(16))):
    """
    Ablauf:
    ...
    Gibt folgende Werte zurück:
      ...
      best_tree, trees_above_threshold_with_depths
    """
    ...
    # Fortschrittsanzeige für Datensammlung
    ...
    depths = range(1,16)  # Beispiel: Tiefe 1..7
    mean_rewards = []
    std_rewards = []
    seeds_mean_rewards = []
    seeds_std_rewards = []
    ...
    # Äußere Fortschrittsanzeige über die Tiefen
    for depth in tqdm(depths, desc="Train/Eval Decision Trees mit verschiedenen Tiefen"):
        ...
        # Für die innere Schleife (Seeds × Episoden):
        num_inner_loops = len(seeds) * n_episodes
        # Innerer Fortschrittsbalken zeigt, wie viele Seeds+Episoden bereits evaluiert wurden
        with tqdm(total=num_inner_loops, desc=f"Evaluating Depth={depth}", leave=False) as pbar:
            for s in seeds:
                ...
                for _ in range(n_episodes):
                    ...
                    for _step in range(1000):
                        # Zustand in DataFrame zur Vorbereitung
                    ...
                    # Fortschrittsanzeige updaten, sobald die Episode abgeschlossen ist
                    pbar.update(1)
...
def main():
    ...
    depths, rew, std, seeds_mean, seeds_std, best_tree, good_trees = gather_performance(
        ...
        num_samples=10000,
        n_episodes=10,      # z.B. 10 Episoden pro Seed
        seeds=list(range(10))  # z.B. Seeds 0..4
    )
