...
class LogProbQ:
    ...
    def q(self, s):
        s = torch.Tensor(s).to(device)  # statt .to(cuda)
        s_repeat = s.repeat(self.env.action_space.n, 1).to(device)
        with torch.no_grad():
            _, s_a_log_probs, _ = self.pol.policy.evaluate_actions(s_repeat, torch.arange(self.env.action_space.n).reshape(-1, 1).to(device))
